...
Assert-True ($repairScriptText -match 'agents\.list\[0\]\.subagents\.allowAgents\[0\]", "\*"') "repair-auth.ps1 should preserve Jarvis cross-agent orchestration"
...
Assert-True ($startScriptText -match "/boards") "start.ps1 should call Mission Control boards API in seed logic"
Assert-True ($startScriptText -match "Perspective:") "start.ps1 should map perspective into board description"
